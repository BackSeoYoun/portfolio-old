...
window.addEventListener("load",function(){
		...
		var move=slider.children[1];
		var prev=slider.children[2].children[0];
		prev.style.opacity=0.4;
		var next=slider.children[2].children[1];
		...
		// 배너 슬라이더
		var sliderB=document.getElementById("slider_content");
		var prevB=sliderB.children[0].children[0];
		var nextB=sliderB.children[0].children[1];
		var moveB=sliderB.children[1].children[0]; // 변경
		var moveBli=moveB.children; // 변경
		var sliderNum=4;
		...
		prevB.addEventListener("click",function(e){
			...
			pos=n*-1*moveBli[0].offsetWidth;
			moveB.style.left=pos+"px";
